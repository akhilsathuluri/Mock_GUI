...
void BGAPI2CALL DevEventHandler( void * callbackOwner, Events::DeviceEvent * pDevEvent)
{
    if ( NULL != pDevEvent )
    {
        ...
        {
            try { gfMessageTimestampMicroSeconds_ExposureStart = (bo_double)pDevEvent->GetTimeStamp() / gfTimestampTickFrequency * 1000000.0; }
            ...
            std::cout << " EventID " << sEventId.c_str() << " " << std::setw(14) << sEventName.c_str() << " Timestamp " << std::fixed << std::setprecision(1) << gfMessageTimestampMicroSeconds_ExposureStart << " [usec]" << std::endl;
        }
        else if (pDevEvent->GetName() == "ExposureEnd")
        {
            try { gfMessageTimestampMicroSeconds_ExposureEnd = (bo_double)pDevEvent->GetTimeStamp() / gfTimestampTickFrequency * 1000000.0; }
            ...
            std::cout << " EventID " << sEventId.c_str() << " " << std::setw(14) << sEventName.c_str() << " Timestamp " << std::fixed << std::setprecision(1) << gfMessageTimestampMicroSeconds_ExposureEnd << " [usec]" << std::endl;

            //CALCULATE TIME DIFFERENCE EXPOSURE END - START
            gfMessageTimestampMicroSeconds_ExposureDiff = gfMessageTimestampMicroSeconds_ExposureEnd - gfMessageTimestampMicroSeconds_ExposureStart;
            std::cout << " Timestamp ExposureEnd - ExposureStart: " << std::fixed << std::setprecision(1) << gfMessageTimestampMicroSeconds_ExposureDiff << " [usec]" << std::endl;
        }
