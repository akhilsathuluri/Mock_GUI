...
int main()
{
    ...
    try
    {
        for (DeviceList::iterator devIterator = deviceList->begin(); devIterator != deviceList->end(); devIterator++)
        {
            try
            {
                ...
                if(devIterator->second->GetRemoteNodeList()->GetNodePresent("ChunkSelector") == false)
                {
    ...
    try
    {
        ...
        if(pDevice->GetRemoteNode("ChunkSelector")->GetEnumNodeList()->GetNodeCount() > 2)
        {
            ...
            std::cout << "        ChunkModeActive:          " << pDevice->GetRemoteNode("ChunkModeActive")->GetValue() << std::endl;

            for(bo_uint64 i = 0; i < pDevice->GetRemoteNode("ChunkSelector")->GetEnumNodeList()->GetNodeCount(); i++)
            {
                BGAPI2::Node * pNodeEnum = pDevice->GetRemoteNode("ChunkSelector")->GetEnumNodeList()->GetNodeByIndex(i);
                if (pNodeEnum->IsReadable() && pDevice->GetRemoteNode("ChunkSelector")->IsWriteable())
                {
                    pDevice->GetRemoteNode("ChunkSelector")->SetString(pNodeEnum->GetString());

                    std::cout << "        ChunkSelector:            " << pDevice->GetRemoteNode("ChunkSelector")->GetValue() << std::endl;
                    if(pDevice->GetRemoteNode("ChunkEnable")->IsWriteable())
                    {
                        pDevice->GetRemoteNode("ChunkEnable")->SetBool(true);
                    }
                    std::cout << "         ChunkEnable:             " << pDevice->GetRemoteNode("ChunkEnable")->GetValue() << std::endl;
                }
            }
            std::cout << std::endl;
        }
        else
        {
            ...
            if(pDevice->GetRemoteNode("ChunkSelector")->GetEnumNodeList()->GetNodePresent("BaumerImageHeader3"))
            {
                ...
                std::cout << "         ChunkEnable:             " << pDevice->GetRemoteNode("ChunkEnable")->GetValue() << std::endl << std::endl;
            }

            //CHUNK TYPE "FreeFormatHeader3"
            else if(pDevice->GetRemoteNode("ChunkSelector")->GetEnumNodeList()->GetNodePresent("FreeFormatHeader3"))
            {
            ...
            else if(pDevice->GetRemoteNode("ChunkSelector")->GetEnumNodeList()->GetNodePresent("FreeformatHeader2"))
            {
    ...
    try
    {
        for(int i = 0; i < 8; i++)
        {
            ...
            else
            {
                ...
                else if( (bIsAvaliableBaumerImageHeader3 == true) || (bIsAvaliableFreeFormatHeader3 == true) || (bIsAvailableChunkExtendedMode == true) )
                {
                    std::cout << "   Buffer.PixelFormat:            " << pBufferFilled->GetPixelFormat() << std::endl;
                    std::cout << "   Buffer.Width:                  " << pBufferFilled->GetWidth() << std::endl;
                    std::cout << "   Buffer.Height:                 " << pBufferFilled->GetHeight() << std::endl;
                    ...
                    std::cout << "   ChunkNodeTree.Count:           " << pBufferFilled->GetChunkNodeList()->GetNodeCount() << std::endl;
                            ...
                            if(pNode->GetInterface() == "IEnumeration")
                            {
                                ...
                                if(pNode->IsSelector() == true)
                                {
                                    ...
                                    for( bo_uint64 l=0; l < pNode->GetEnumNodeList()->GetNodeCount(); l++)
                                    {
                                        ...
                                        if( (nEnumNode->IsReadable() == true) && (nEnumNode->GetVisibility() != "Invisible") )
                                        {
                                            ...
                                            std::cout << " " << std::left << std::setw(33) << pNode->GetName() << ": " << pNode->GetEnumNodeList()->GetNodeByIndex(l)->GetValue() << std::right << std::endl;
                                            ...
                                            for(bo_uint64 s=0; s < nSelectedNodeList->GetNodeCount(); s++)
                                            {
                                                ...
                                                {
                                                    std::cout << "                        " << std::left << std::setw(32) << nSelectedNode->GetName() << std::right << ": " << nSelectedNode->GetValue() << std::endl;
                                                }
                                            }
                                        }
                                    }
                ...
                else if( bIsAvaliableFreeformatHeader2 == true )
                {
                    ...
                    {
                        std::cout << "    Chunk.ExposureTime:           " << pFreeformatHeader2->uiExposureValue << std::endl;
                        std::cout << "    Chunk.Gain:                   " << pFreeformatHeader2->uiGain << std::endl;
                        std::cout << "    Chunk.BlackLevelOffset:       " << pFreeformatHeader2->uiOffset << std::endl;
                        std::cout << "    Chunk.OffsetX:                " << pFreeformatHeader2->usFrameStartPositionX << std::endl;
                        std::cout << "    Chunk.OffsetY:                " << pFreeformatHeader2->usFrameStartPositionY << std::endl;
                        std::cout << "    Chunk.Width:                  " << pFreeformatHeader2->usFrameExtensionX << std::endl;
                        std::cout << "    Chunk.Height:                 " << pFreeformatHeader2->usFrameExtensionY << std::endl;
                        std::cout << "    Chunk.FrameCounter:           " << pFreeformatHeader2->uiFrameCounter << std::endl;
                        std::cout << "    Chunk.PixelFormatHex:         0x" << std::hex << pFreeformatHeader2->uiDataFormat << std::dec << std::endl;
