/*
    ...
    If just one of the available polarisation data format is needed it is more efficient to use
    a single-part image for the calculation. This is described in the example 020_Polarized_SinglePart.cpp
*/
...
static void GetSupportedComponents(BGAPI2::Polarizer* const polarizer
    , std::map<std::string, BGAPI2::Polarizer::Formats>* const supportedComponents, std::string* const help);
...
static int ReleaseAllResources(BGAPI2::System* pSystem, BGAPI2::Interface* pInterface, BGAPI2::Device* pDevice
    , BGAPI2::DataStream* pDataStream, std::map<BGAPI2::Polarizer::Formats, BGAPI2::Image*>* requestedComponents);
...
int main(int argc, char* argv[])
{
    ...
    std::cout << std::endl;
    std::cout << "################################################" << std::endl;
    std::cout << "# PROGRAMMER'S GUIDE Example 022_Polarized.cpp #" << std::endl;
    std::cout << "################################################" << std::endl;
    ...
    std::map<std::string, BGAPI2::Polarizer::Formats> supportedComponents;
    std::string help;

    GetSupportedComponents(&polarizer, &supportedComponents, &help);
    help = "enable component (" + help + ")";

    bool enableInterpolation = false;

    static const Argument argumentList[] = {
        { &enableInterpolation, "i",  "interpolate", true,  argumentBoolFlag,  false, "<on/off>",     "enable or disable interpolation" },
    ...
    try
    {
        for (std::set<std::string>::const_iterator component = sComponents.begin();
            ...
            if (c == supportedComponents.end())
            {
                ...
                requestedComponents.insert(std::pair<BGAPI2::Polarizer::Formats, BGAPI2::Image*>(c->second, imgProc.CreateImage()));
            }
        }
    }
    catch (BGAPI2::Exceptions::IException& ex) {
        ...
        for (int i = 0; i < 12; i++)
        {
            ...
            else
            {
                ...
                if (getResult)
                {
                    try
                    {
                        for (std::map<BGAPI2::Polarizer::Formats, BGAPI2::Image*>::iterator component = requestedComponents.begin();
...
void argumentComponent(const Argument& argument, const ArgumentMode mode, const char* const pParam)
{
    ...
    {
        sComponents.clear();
        sComponents.insert("Intensity");
        sComponents.insert("AOP");
        sComponents.insert("DOLP");
        sComponents.insert("ADOLP");
