...
void BGAPI2CALL DevEventHandler(void * callbackOwner, Events::DeviceEvent * pDevEvent);
...
int main(int argc, char* argv[])
{
    ...
    try
    {
        for (DeviceList::iterator devIterator = deviceList->begin(); devIterator != deviceList->end(); devIterator++)
        {
            try
            {
                ...
                if (devIterator->second->GetAccessStatus() == "RW") {
                    devIterator->second->Open();
                    // enable multicast
                    devIterator->second->GetNode("MulticastStream")->SetBool(true);
                    devIterator->second->GetNode("MulticastMessage")->SetBool(true);
                    bMaster = true;
                } else {
                    devIterator->second->OpenReadOnly();
                    bMaster = false;
                    // WARNING ACCESS IS NOT THREAD SAVE - YOU SHOULD ALWAYS USE A LOCK (LIKE MUTEX) TO ACCESS THEM FROM DIFFERENT THREADS
                    // This example does not use std::mutex to support old compiler without C++11
                    bOutput = false;
    ...
    if (pDevice->GetRemoteNodeList()->GetNodePresent("EventSelector")) {
        try {
            ...
            pDevice->RegisterDeviceEventHandler(pDevice, (Events::DeviceEventHandler) &DevEventHandler);
            std::cout << std::endl;
        }
        catch (BGAPI2::Exceptions::IException& ex) {
            returncode |= 1;
    ...
    if (bMaster) {
        ...
        std::cout << "###########################" << std::endl << std::endl;
        std::cout << "This instance has write access to the camera and can therefore configure the camera and start/stop the stream." << std::endl;
        std::cout << "A second instance of this example, when started concurrently, will start as a read-only receiver." << std::endl;
    } else {
        std::cout << "Camera Listening Instance" << std::endl;
        std::cout << "#########################" << std::endl << std::endl;
        std::cout << "This instance has read-only access to the camera and can therefore read camera parameters but not change any settings." << std::endl;
    }
...
void BGAPI2CALL BufferHandler(void * callBackOwner, Buffer * pBufferFilled) {
    ...
    sOutput << "Input any number to stop [callback of " << ((BGAPI2::DataStream *) callBackOwner)->GetParent()->GetModel() << "] ";  // device
    ...
    try {
        ...
        else
        {
            ...
            if (pBufferFilled->GetIsIncomplete() == true) {
                ...
                sOutput << " Image " << std::setw(5) << pBufferFilled->GetFrameID() << " received in memory address " << std::hex << pBufferFilled->GetMemPtr() << std::dec << std::endl;
            }
            // queue buffer again
...
void BGAPI2CALL DevEventHandler(void * callbackOwner, Events::DeviceEvent * pDevEvent) {
    ...
    if (NULL != pDevEvent) {
        ...
        try { fMessageTimestampMicroSecond = (bo_double)pDevEvent->GetTimeStamp() / gfTimestampTickFrequency * 1000000.0; }
        ...
        sOutput << " EventID " << sEventId.c_str() << " " << std::setw(14) << sEventName.c_str() << " Timestamp " << std::fixed << std::setprecision(1) << fMessageTimestampMicroSecond << " [usec]" << std::endl;
    } else {
