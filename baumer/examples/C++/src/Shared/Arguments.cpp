...
void argumentBoolFlag(const Argument& argument, const ArgumentMode mode, const char* pParam) {
    if (mode == eArgumentInit) {
        *((bool*)(argument.pData)) = (argument.value != 0) ? true : false;
    } else {
        *((bool*)(argument.pData)) = (mode == eArgumentAdd) ? true : false;
    }
}
...
bool isBoolParam(const char* const pParam, bool& bValue, const char* const pParamOn, const char* const pParamOff) {
    ...
    std::transform(sOn.begin(), sOn.end(), sOn.begin(), ::tolower);

    std::string sOff = (pParamOff != NULL) ? pParamOff : "off";
    std::transform(sOff.begin(), sOff.end(), sOff.begin(), ::tolower);
...
static bool processArgument(const char* pString, const Argument* pArgumentList, const unsigned int argumentCount
                          , unsigned int* const pIndex = NULL, bool bSkip = false) {
...
static size_t getHelpFieldLength(const Argument* const pArgumentList, const unsigned int argumentCount
...
static void displayHelp(const Argument* const pArgumentList, const unsigned int argumentCount, const std::string sBaseShort,
                        const std::string sBaseFull, const size_t fieldLength, unsigned int * const pIndex = NULL) {
    ...
    while (i < argumentCount) {
        if (pArgumentList[i].pProcessArgument != NULL) {
            if ((pArgumentList[i].pDescription1 != NULL) && (pArgumentList[i].pDescription2 != NULL)) {
                ...
                /*
                sArgumentFull = sBaseShort + pArgumentList[i].pName;
                sArgumentFull += ":";
                sArgumentFull += pArgumentList[i].pDescription1;
                std::cout << "-" << std::setw(fieldLength) << std::left << sArgumentString << " - " <<
                pArgumentList[i].pDescription2 << std::endl;
                */
            }
        } else if (pArgumentList[i].pName != NULL) {
