...
int main()
{
    ...
    try
    {
        ...
        {
            //DEVICE INFORMATION
            std::cout << "    Device DeviceID:                       " << pDevice->GetUpdateNode("DeviceID")->GetValue() << std::endl;
            std::cout << "    Device Model:                          " << pDevice->GetUpdateNode("DeviceModelName")->GetValue() << std::endl;
            std::cout << "    Device SerialNumber:                   " << pDevice->GetUpdateNode("DeviceSerialNumber")->GetValue() << std::endl;
            std::cout << "    Device Product ID:                     " << pDevice->GetUpdateNode("DeviceProductID")->GetValue() << std::endl;
            std::cout << "    Device Config ID (Firmware Version):   " << pDevice->GetUpdateNode("DeviceConfigID")->GetValue() << std::endl;
            ...
            std::cout << "    Path to Update Files:                  " << pDevice->GetUpdateNode("UpdatePath")->GetValue() << std::endl;
            ...
            for (int i = 0; i < iUpdateNumber; i++)
            {
                ...
                std::cout << "        Updateversion:             [index " << std::setw(2) << i << "]:   " << pDevice->GetUpdateNode("ConfigIDName")->GetValue();
        ...
        try
        {
            ...
            if (pDevice->IsUpdateModeActive())
            {
                if (pDevice->GetUpdateNode("ConfigIDSelector")->GetAvailable())
                {
                    ...
                    std::cout << "    Selected Device Config ID (Firmware Version):   " << pDevice->GetUpdateNode("ConfigIDName")->GetValue() << std::endl;
                }
                std::cout << "    Start update!" << std::endl;
                pDevice->GetUpdateNode("StartUpdate")->Execute();
                while (pDevice->GetUpdateNode("DeviceState")->GetValue() == "Updating")
                {
                    std::cout << "    Update Progress:  " << std::setw(4) << pDevice->GetUpdateNode("UpdateProgress")->GetValue() << "%" << '\r' << std::flush;
#if defined(_WIN32) // predefined Macro for ALL VC++ projects!
                    Sleep(1000);
#else
                    usleep(1000000);
                ...
                std::cout << "    Update finished with result: " << pDevice->GetUpdateNode("UpdateResult")->GetValue() << std::endl;
            }
        }
        catch (BGAPI2::Exceptions::IException& ex)
        {
        ...
        try
        {
            ...
            if (pDevice->IsUpdateModeActive())
            {
                if (pDevice->GetUpdateNode("StartReboot")->GetAvailable())
                {
                    ...
                    std::cout << std::endl << "    Reboot finished with result: " << pDevice->GetUpdateNode("RebootResult")->GetValue() << std::endl;
                }
                else {
